                ...
                <div class="box data-table-list">
                    ...
                    <div class="box-body no-padding">
                        <div class="basic-tb-hd clearfix">
                            <h2 class="panel-title pull-left" style="padding-top: 7.5px;">Toppics<br><span
                            ...
                            <div class="btn-group pull-right">
                                <a href="{{action('ToppicsController@create')}}" class="btn btncus btn_blue"><i
                                        class="notika-icon notika-draft"></i> Add New Toppic</a>
                            </div>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-striped">
                                ...
                                <tr>
                                    ...
                                    <th class="col-md-2">Show Home</th>
                                ...
                                @foreach($pages as $k=>$row)
                                    <tr id="{{ $row->id }}" class="ui-sortable-handle">
                                        <td>{{$row->id}}</td>
                                        <td><img src="{{ asset('public/images/toppics') }}/{{$row->images}}" width="150px" alt="Product"
                                                 class="img-fluid"></td>
                                        <td>{{$row->title}}</td>
                                        <td>{{($row->home ==1)?'Yes':'No'}}</td>
                                        <td>{{($row->status ==0)?'In Active':'Active'}}</td>
                                        <td style="text-align: right"><a
                                                href="{{action('ToppicsController@edit', $row->id)}}"
                                                class="btn btncus btn_blue"><i class="notika-icon notika-draft"></i>
                                                Edit
                                            </a>
                                            <a href="javascript:void(0);"
                                               data-url="{{action('ToppicsController@destroy', $row->id)}}"
                                               data-id="{{$row->id}}"
                                               class="_delete_data">
            ...
            $(".sortable-posts").sortable({
                stop: function () {
                    ...
                    $.map($(this).find('tr'), function (el) {
                        ...
                        var sort = $(el).index();
                        $.ajax({
                            url: '{{action('ToppicsController@order')}}',
                            type: 'GET',
                            data: {
                                id: id,
                                sort: sort
